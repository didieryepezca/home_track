...
{
    public class UsuarioNuevoDTOFluentValidator : AbstractValidator<UsuarioNuevoDTO>
    {
        public UsuarioNuevoDTOFluentValidator()
        {
        ...
        {
            var result = await ValidateAsync(ValidationContext<UsuarioNuevoDTO>.CreateWithOptions((UsuarioNuevoDTO)model, x => x.IncludeProperties(propertyName)));
            if (result.IsValid)
